#!/bin/sh
set -eu
# Continuous WildRig 0.51.3 for the existing PearlHash multipool arm.
...
case "${BURNIN_SECONDS:-0}" in 0|continuous|infinite) ;; *) echo 'canary image requires continuous mode' >&2; exit 64;; esac
...
export MINER_KIND=wildrig MINER_VERSION=0.51.3 MINER_API_ENABLED=0
...
printf '{"event":"miner_start","schema":"miner_telemetry.v1","arm":"%s","pool":"%s","worker":"%s","miner":"wildrig","miner_version":"0.51.3","source":"container_stdout"}\n' \
